...
def Resource_Allocation(matrix, pred_index):
    # 计算RA指标
    add_row = np.sum(matrix, axis=1)  # 行求和
    add_row = add_row[:, np.newaxis]
    sim_matrix = matrix / add_row  # 节点权重
...
def sim_GNN(model,node_num, train_num, edge_index, test_mask, train_mask, zero_index):
    def train(x):
        ...
        neg_edge_index = negative_sampling(
            ...
            num_neg_samples=train_num, method='sparse')  # num_neg_samples=train_num
        ...
        edge_label = torch.cat([
            torch.ones(train_num), torch.zeros(train_num)  # torch.zeros(train_num)
        ], dim=0).to(device)
    ...
    criterion = torch.nn.BCELoss() #criterion = torch.nn.BCEWithLogitsLoss()
    if train_num > 0:
...
# train_edge = np.array([[2, 5, 1, 4, 5, 3, 1, 3], [1, 1, 2, 3, 3, 4, 5, 5]])
# for edge in train_edge.T:
#     train_data[edge[0] - 1][edge[1] - 1] = 1
#     train_data[edge[1] - 1][edge[0] - 1] = 1
#
# print(CH2_AND_L3(train_data))
# print()
# num_nodes = 10
# upper_triangular = np.random.randint(0, 2, size=(num_nodes, num_nodes))
...
# print()
# print(train_data)
# indices = np.triu_indices(train_data.shape[0], k=1)
# zero_indices = (indices[0][train_data[indices] == 0], indices[1][train_data[indices] == 0])
# print(zero_indices)

# edge_index=[[0,3],[1,3],[2,3],[4,5],[6,7],[6,8],[10,11],[10,12],[3,6],[3,10],[3,9],[9,10],[4,6],[4,9],[6,9]]
# A = np.zeros((13,13), dtype=int)
# for [x, y] in edge_index:
#     A[x][y] = 1
#     A[y][x] = 1
# pred_index=np.array([[6,10]])
# print(Cannistraci_Hebb_L2_L3(A,pred_index))
