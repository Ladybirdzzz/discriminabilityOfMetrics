...
import lp_numpy as lp
from METRICS_new import prediction_evaluation
...
for row in worksheet.iter_rows(min_row=2, values_only=True):
    ...
    path = 'result_0611/{0}/'.format(network)
    file_dict = {}
    for alg in algorithms:
        if not os.path.exists(path + alg):
            os.makedirs(path + alg)
        file_path = 'result_0611/{0}/{1}/details.txt'.format(network, alg)
